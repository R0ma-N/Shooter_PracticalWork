...
namespace BrokenVector.TreeRandomizer
{
    public class TreeRandomizerWindow : EditorWindow
    {
        ...
        private const string OUTPUT_FOLDER = "Generated";
        private const int SLIDER_TREE_COUNT_MAX = 50;

        private Tree treeTemplate = null;
        private int treeCount = 5;

        [MenuItem("Tools/Tree Randomizer")]
        private static void ShowWindow()
        {
            var window = GetWindow(typeof(TreeRandomizerWindow));
            window.titleContent = new GUIContent("Tree Randomizer");
            window.Show();
        }
        ...
        void OnGUI()
        {
            ...
            treeCount = EditorGUILayout.IntSlider("Tree Count", treeCount, 1, SLIDER_TREE_COUNT_MAX);
        ...
        private void Generate(Tree treeTemplate, int treeCount)
        {
            ...
            string copyFolder = dir + "/" + OUTPUT_FOLDER;

            if (!AssetDatabase.IsValidFolder(copyFolder))
                AssetDatabase.CreateFolder(dir, OUTPUT_FOLDER);
